<template>
  <div class="ModalContainer">
    <img class="logo" src="../assets/img/letras.png" alt="Plomero" />
    <InputData placeholder="Email" widthInput="width: 350px;" />
    <div>
      <InputData placeholder="First name" widthInput="width: 120px;" />
      <InputData
        placeholder="Last name"
        widthInput="width: 120px;"
        style="margin-left: 50px"
      />
    </div>
    <InputData placeholder="Phone" widthInput="width: 350px;" />
    <InputData placeholder="Create a Password" widthInput="width: 350px;" />
    <InputData placeholder="Zip Code" widthInput="width: 350px;" />

    <p class="fontBlack">
      By clicking below and creating an account,<p class="blue fontBold">I agree to ProviTask's Terms of
      Service and Privacy Policy.</p> 
    </p>
    <button class="fontBold" @click="registerClient()">CREATE ACCOUNT</button>
  </div>
</template>
<script>
import InputData from "@/components/InputData.vue";
export default {
  name: "ModalContainer",
  components: {
    InputData,
  },
  data() {
    return {};
  },
  methods: {
    async registerClient() {
      // Ejemplo para recibir los datos del state
      console.log(
        this.$store.state.email,
        this.$store.state.firstName,
        this.$store.state.lastName,
        this.$store.state.phone,
        this.$store.state.password,
        this.$store.state.zipCode
      );
      // Fin del ejemplo
      const response = await fetch("http://3.87.96.160:1337/graphql", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization:
            "Bearer ",
        },
        body: JSON.stringify({
          query: `
        mutation RegistroCliente(
          $username: String!
          $email: String!
          $password: String!
          $name: String!
          $last_name: String!
          $postal_code: String!
          $phone: String!
          $token: String
        ) {
          register(input: {
            username: $username
            email: $email
            password: $password
          }){
            user{
              username
            }
          }
          createClient(data: {
            name: $name,
            last_name: $last_name,
            email: $email,
            password: $password,
            postal_code: $postal_code,
            username: $username,
            token: $token,
            phone: $phone
          }) {
            data {
              id
              attributes {
                email
              }
            }
          }
        }`,
          variables: {
            username: this.$store.state.firstName + this.$store.state.lastName + this.$store.state.zipCode,
            email: this.$store.state.email,
            password: this.$store.state.password,
            name: this.$store.state.firstName,
            last_name: this.$store.state.lastName,
            postal_code: this.$store.state.zipCode,
            phone: this.$store.state.phone,
            token: "1234",
          },
        }),
      })
        .then((res) => res.json())
        .then((result) => {
          if(result.data != ""){
            alert("Usuario creado con exito");
            window.location.href = "../#/login";
          }
        });

      return response;
    },
  },
};
</script>
<style scoped>
p {
  width: 550px;
}
button {
  background: #1b0493;
  color: white;
  border: none;
  padding: 10px 15px;
  font-size: 34px;
  border-radius: 10px;
}
.logo {
  width: 400px;
  height: 100px;
}
.ModalContainer {
  padding: 30px 0;
  margin-top: 100px;
  border-radius: 40px;
  width: 50vw;
  height: 800px;
  box-shadow: 0 0 10px black;
  display: flex;
  flex-direction: column;
  justify-content: space-around;
  align-items: center;
  background: white;
  margin-bottom: 50px;
}
</style>
